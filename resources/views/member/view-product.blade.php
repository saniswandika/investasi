            ...
            <div class="d-flex align-items-start">
                <div class="detail-content--left flex-shrink-0y">
                    <div class="user-detail text-center pr-5">
                        <div class="user-avatar shadow" style="background-image: url('@if (empty(Auth::user()->photo)) {{url('img/default-image.png')}} @else {{url('')}}/users/thumbnail/{{Auth::user()->photo}} @endif');"></div>
                        <div class="d-block mb-3">
                            <div class="font-semibold">{{Auth::user()->name}}</div>
                            <div class="font-s-small">{{Auth::user()->alamat}}</div>
                        </div>
                        <a href="{{route('front.companiesshow',['slug'=>Auth::user()->slug])}}" class="button button--small button--primary">PROFILE VISIT</a>
                    </div>
                </div>
                <div class="detail-content--right flex-shrink-0">
                    <div>
                        <h1 class="font-semibold mb-4 d-block">{{$product->name}}</h1>
            ...
            <div class="card-group-custom card-group-shadow card-group-25 card-group-vertical">
                ...
                <div class="card-item">
                    <div class="card-item--inner card-item--listing">
                        <div class="card-item--listing-image" style="background-image: url('{{url('')}}/img/product/thumbnail/{{$p->image}}')"></div>
                        ...
                        <div class="card-item--listing-content">
                            ...
                            <div>
                                ...
                                <a href="{{route('member.show',['id'=>$p->id])}}" class="button-text mb-1">VIEW PRODUCT</a>
                            </div>
                        </div>
                    </div>
                </div>
