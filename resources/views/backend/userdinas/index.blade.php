            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>Daftar User Dinas</h3>
                        <a href="/userdinas/create" class="btn btn-sm btn-primary ml-auto">Add User Dinas</a>
                    </div>
                </div>
                <div class="table-responsive py-4">
                    <table class="table table-flush" id="datatable-buttons">
                        ...
                        <tbody>
                        @foreach($userdinas as $user)
                            <tr>
                                <td>{{$user->id}}</td>
                                <td>{{$user->nama_dinas}}</td>
                                <td><img class="img-table" src="@if (empty($user->logo_dinas))
                                    {{url('img/default-image.png')}}
                                    @else
                                    {{url('')}}/users/thumbnail/{{$user->logo_dinas}}
                                    @endif" alt=""></td>
                                <td>{{$user->name}}</td>
                                <td>{{$user->telp}}</td>
                                <td>{{$user->created_at}}</td>
                                <td>
                                    <a href="{{route('userdinas.show',['id'=>$user->id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                    <a href="{{route('userdinas.edit',['id'=>$user->id])}}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('userdinas.destroy',['id'=>$user->id])}}" class="btn btn-danger" onclick="return confirm('are you sure want to delete ?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
