            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>Commodities</h3>
                        <a href="{{route('comodities.create')}}" class="btn btn-sm btn-primary ml-auto">Tambah</a>
                    </div>
                </div>
                        ...
                        @foreach($comodities as $i)
                            <tr>
                                ...
                                <td><img class="img-table" src="@if (empty($i->image))
                                    ...
                                    @else
                                    {{url('')}}/img/commodities/thumbnail/{{$i->image }}
                                    @endif" alt=""></td>
                                <td>
{{--                                    <a href="{{route('industri.show',['id'=>$i->id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>--}}
                                    <a href="{{route('comodities.edit',['id'=>$i->id])}}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('comodities.destroy',['id'=>$i->id])}}" class="btn btn-danger" onclick="return confirm('are you sure want to delete ?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
