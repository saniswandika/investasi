            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>Halaman Daftar Publikasi</h3>
                        <a href="/../public/publikasi/create" class="btn btn-sm btn-primary ml-auto">Tambah Publikasi</a>
                    </div>
                </div>
                <div class="table-responsive py-4">
                    <table class="table table-flush" id="datatable-buttons">
                        ...
                        <tbody>
                        @foreach($publikasi as $f)
                            <tr>
                                <td>{{$f->pub_id}}</td>
                                <td>{{$f->judul}}</td>
                                <td>
                                    <a class="btn btn-primary"
                                    href="{{url('')}}/pub/file/{{$f->file}}" target="_blank">Download File</a>
                                </td>
                                <td><img class="img-table" src="@if (empty($f->image))
                                    {{url('img/default-image.png')}}
                                    @else
                                    {{url('')}}/img/pub/thumbnail/{{$f->image}}
                                    @endif" alt=""></td>
                                <td>{{$f->name}}</td>
                                <td>{{$f->created_at}}</td>
                                <td>
                                    <a href="{{route('publikasi.show',['pub_id'=>$f->pub_id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                    <a href="{{route('publikasi.edit',['pub_id'=>$f->pub_id])}}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('publikasi.destroy',['pub_id'=>$f->pub_id])}}" class="btn btn-danger" onclick="return confirm('are you sure want to delete ?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
