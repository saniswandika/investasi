            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>New Industrial Zone</h3>
                        <a href="{{route('industri-new.create')}}" class="btn btn-sm btn-primary ml-auto">Tambah Industri</a>
                    </div>
                </div>
                        ...
                        @foreach($industri as $i)
                            <tr>
                                ...
                                <td><img class="img-table" src="@if (empty($i->image))
                                    ...
                                    @else
                                    {{url('')}}/img/new-industry/thumbnail/{{$i->image }}
                                    @endif" alt=""></td>
                                <td>
{{--                                    <a href="{{route('industri.show',['id'=>$i->id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>--}}
                                    <a href="{{route('industri-new.edit',['id'=>$i->id])}}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('industri-new.destroy',['id'=>$i->id])}}" class="btn btn-danger" onclick="return confirm('are you sure want to delete ?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
