            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>Halaman Economy Statistic</h3>
                        <a href="/../public/economyStatistic/create" class="btn btn-sm btn-primary ml-auto">Tambah Konten</a>
                    </div>
                </div>
                <div class="table-responsive py-4">
                    <table class="table table-flush" id="datatable-buttons">
                        ...
                        <tr>
                            <th>Id</th>
                            <th>Inflation</th>
                            <th>Inflation Date</th>
                            <th>Economic Growth</th>
                            <th>Economic Growth Date</th>
                            <th>West Java Export</th>
                            <th>West Java Export Date</th>
                            <th>Action</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($economyStatistic as $a)
                            <tr>
                                <td>{{$a->Id}}</td>
                                <td>{{$a->inflation}}</td>
                                <td>{{$a->inflationDate}}</td>                               
                                <td>{{$a->economicGrowth}}</td>
                                <td>{{$a->economicGrowthDate}}</td>
                                <td>{{$a->westJavaExport}}</td>
                                <td>{{$a->westJavaExportDate}}</td>
                                <td>
                                    <a href="{{route('economyStatistic.show',['Id'=>$a->Id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                    <a href="{{route('economyStatistic.edit',['Id'=>$a->Id])}}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('economyStatistic.destroy',['Id'=>$a->Id])}}" class="btn btn-danger" onclick="return confirm('are you sure want to delete ?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
