            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>Halaman Daftar Agenda</h3>
                        <a href="/../public/agenda/create" class="btn btn-sm btn-primary ml-auto">Tambah Konten Agenda</a>
                    </div>
                </div>
                    ...
                    <table class="table table-flush" id="datatable-buttons">
                        ...
                        <tr>
                                ...
                                <td><img class="img-table" src="@if (empty($a->image))
                                    ...
                                    @else
                                    {{url('')}}/img/agenda/thumbnail/{{$a->image}}
                                    @endif" alt=""></td>
                                    <td>{{$a->place}}</td>
                                    <td>{{$a->eventDate}}</td>
                                <td>@if($a->isBahasa == 0)
                                ...
                                <td>
                                    <a href="{{route('agenda.show',['Id'=>$a->Id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                    <a href="{{ route('agenda.edit', ['Id' => $a->Id]) }}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('agenda.destroy',['Id'=>$a->Id])}}" class="btn btn-danger" onclick="return confirm('are you sure want to delete ?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
