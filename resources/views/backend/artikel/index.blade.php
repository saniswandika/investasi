            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>Halaman Daftar Artikel</h3>
                        <a href="/../public/artikel/create" class="btn btn-sm btn-primary ml-auto">Tambah Artikel</a>
                    </div>
                </div>
                            ...
                            @foreach ($artikel as $a)
                                <tr>
                                    ...
                                    <td><img class="img-table"
                                            src="@if (empty($a->image)) {{ url('img/default-image.png') }}
                                    @else
                                    {{ url('') }}/img/artikel/thumbnail/{{ $a->image }} @endif"
                                            alt=""></td>
                                    <td>
                                        @if ($a->lang_type == 'id')
                                            Indonesia
                                        @elseif($a->lang_type == 'en')
                                            Inggris
                                        @else
                                            No Language
                                        @endif
                                    ...
                                    <td>{{ $a->created_at }}</td>
                                    <td>
                                        <a href="{{ route('artikel.show', ['art_id' => $a->art_id]) }}"
                                            class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                        <a href="{{ route('artikel.edit', ['art_id' => $a->art_id]) }}"
                                            class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                        <a href="{{ route('artikel.destroy', ['art_id' => $a->art_id]) }}"
