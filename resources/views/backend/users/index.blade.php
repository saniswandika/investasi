            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>Users</h3>
                        <a href="{{route('users.create')}}" class="btn btn-sm btn-primary ml-auto">Add Users</a>
                    </div>
                </div>
                        ...
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td><img class="img-table" src="@if (empty($user->photo))
                                    ...
                                    @endif" alt=""></td>
                                <td>{{ Str::limit(html_entity_decode (strip_tags($user->description)),100)}}</td>
                                <td>{{$user->created_at}}</td>
                                <td>
                                    <a href="{{route('users.show',['id'=>$user->id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                    <a href="{{route('users.edit',['id'=>$user->id])}}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('users.destroy',['id'=>$user->id])}}" class="btn btn-danger" onclick="return confirm('are you sure?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
