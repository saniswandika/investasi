            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>List Profile</h3>
                        <a href="/../public/profile/create" class="btn btn-sm btn-primary ml-auto">Tambah profile</a>
                    </div>
                </div>
                        ...
                        @foreach($profile as $i)
                            <tr>
                                ...
                                <td><img class="img-table" src="@if (empty($i->image))
                                    ...
                                    @else
                                    {{url('')}}/img/profile/thumbnail/{{$i->image}}
                                    @endif" alt=""></td>
                                <td>
                                    <a href="{{route('profile.show',['id'=>$i->id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                    <a href="{{route('profile.edit',['id'=>$i->id])}}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('profile.destroy',['id'=>$i->id])}}" class="btn btn-danger" onclick="return confirm('are you sure want to delete ?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
