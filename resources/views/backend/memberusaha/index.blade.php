            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row align-items-center">
                        <h3>Daftar Member Usaha</h3>
                        <a href="/memberusaha/create" class="btn btn-sm btn-primary ml-auto">Add Member Usaha</a>
                    </div>
                </div>
                <div class="table-responsive py-4">
                    <table class="table table-flush" id="datatable-buttons">
                        ...
                        <tbody>
                        @foreach($memberusaha as $user)
                            <tr>
                                <td>{{$user->id}}</td>
                                <td>@if (empty($user->status)) Not Approve @else Approve @endif</td>
                                <td>{{$user->name}}</td>
                                <td><img class="img-table" src="@if (empty($user->photo))
                                    {{url('img/default-image.png')}}
                                    @else
                                    {{url('')}}/users/thumbnail/{{$user->photo}}
                                    @endif" alt=""></td>
                                <td>{{$user->email}}</td>
                                <td>{{$user->telp}}</td>
                                <td>
                                    <a href="{{route('memberusaha.show',['id'=>$user->id])}}" class="btn btn-primary"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                    <a href="{{route('memberusaha.edit',['id'=>$user->id])}}" class="btn btn-warning"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <a href="{{route('memberusaha.destroy',['id'=>$user->id])}}" class="btn btn-danger" onclick="return confirm('are you sure want to delete ?')"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
